...
def decode(data):
    result = b''
    addr = 0
    block_size = 0x08 << data[addr]
    subblock_count = 1 << data[addr]
    addr += 1
    while len(result) < 0x0800:
        default_byte = data[addr:addr+1]
        addr += 1
        
        addr2 = addr + subblock_count
        for n in range(subblock_count):
            bitmask = data[addr]
            addr += 1
            for bit in range(8):
                if bitmask & (0x80 >> bit):
                    result += data[addr2:addr2+1]
                    addr2 += 1
                else:
                    result += default_byte
        addr = addr2
...
if __name__ == "__main__":
    ...
    data = decode(data[0x03 * 0x4000 + 0x1C00:])
    # data = decode(data[0x0D * 0x4000 + 0x0BF8:])
    bank_count = (len(data) + 0x3FFF) // 0x4000
